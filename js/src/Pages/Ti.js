...
const Transcribe = () => {
  ...
  const [currentState, setCurrentState] = useState(newTranscript);
  //------------------------------------------------//
  const [error, setError] = useState(null);
  const [redoTranscriptHistory, setRedoTranscriptHistory] = useState([]);
  ...
  const handleRedoTranscript = () => {
    ...
    if (redoTranscriptHistory.length > 0) {
      const nextTranscript = redoTranscriptHistory[redoTranscriptHistory.length - 1];
      setTranscriptHistory(prevHistory => prevHistory.slice(0, +1));
      setRedoTranscriptHistory(nextTranscript);
 ...
 <textarea
                rows={10}
                className="from-text"
                defaultValue={newTranscript}
                onChange={(e) => setNewTranscript(e.target.value)}
                onInput={handleTeaxtareaChange}
              />                  
                </>
              ) : (
                "Hold On Button to start..."
           )} */}
              <textarea
                rows={10}
                className="from-text"
